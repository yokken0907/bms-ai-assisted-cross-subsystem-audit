...
本リポジトリは、Keiji Yoshimuraによる論文 **“From Candidate Detection to Bounded Claims: A Reproducible AI-Assisted Cross-Subsystem Audit of Open-Source Battery Management Software”** とSupplement A/B/C（Version 1.1）の再現性資料である。
...
これにより、論文ファイルのSHA、固定集計値、witness hash、25 source anchors、15 candidates + 2 controlsの再実行を確認する。
...
Publication SetとMaster-of-Allを取得した後は、次でcanonical SHAとpaper/の4 DOCX一致を確認できる。

```bash
python scripts/verify_release_assets.py --publication-set BMS_AI_ASSISTED_CROSS_SUBSYSTEM_AUDIT_PUBLICATION_SET_v1.1.zip --master-of-all BMS_TECHNOLOGY_AUDIT_MASTER_OF_ALL_v1.0.0.zip
```
